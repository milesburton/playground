...
import {
  ...
  const POCKET_RADIUS = 15; // Adjust based on table dimensions
  ...
  export class Physics {
    ...
    constructor(dimensions: TableDimensions) {
      ...
      this.pockets = this.initializePockets();
    }

    // Initialize pocket positions (top-left, top-center, top-right, bottom-left, bottom-center, bottom-right)
    private initializePockets(): Vector2D[] {
    ...
    public updateBalls(balls: Ball[]): boolean {
      ...
      for (const ball of balls) {
        if (!ball.isPocketed) {
          ...
          // Check if the ball has entered a pocket
          if (this.isBallInPocket(ball)) {
            ball.isPocketed = true;
            ball.velocity = { x: 0, y: 0 }; // Stop the ball once pocketed
          } else if (this.isBallMoving(ball)) {
            anyBallMoving = true;
          }
        }
      }
    ...
    private isBallInPocket(ball: Ball): boolean {
