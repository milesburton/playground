...
export function checkPocketCollisions(ball: Ball, table: TableDimensions): boolean {
...
export function updateGameState(state: GameState, table: TableDimensions): ShotResult {
...
function checkGameOver(state: GameState): boolean {
  ...
  const playerBalls = state.balls.filter(ball =>
    !ball.isCue &&
    !ball.isPocketed &&
    ball.number !== 8 &&
  ...
  state.winner = playerBalls.length === 0 ? state.currentPlayer : (state.currentPlayer === 1 ? 2 : 1);
  return true;
}
